...
<div class="panel-body">
	...
	<div class="btn-group">
		...
		<?php //close order dilakukan oleh peg dan jika status order dalam tahap diteruskan
		echo $order['idTiketStatus']==3&&$this->session->userdata('idRole')==8 || $order['idTiketStatus']==2&&$this->session->userdata('idRole')==7 || $order['idTiketStatus']==3&&$tutupKbag['dari']==$tutupKbag['kepada']&&$this->session->userdata('idRole')==7?'':'style="display:none"';?> >Tutup Order</button>
		<!-- tombol kabag -->
		...
		echo $order['idTiketStatus']==4&&$this->session->userdata('idRole')==7&&$order['idDepartemenTujuan']==$this->session->userdata('idDepartemen')?'':'style="display:none"'; ?>>Verifikasi Order</button>
		<!--
		if(!empty($disposisi)){
			$no=1;
