...
<table class="table table-condensed" width="100%">
	...
	<div class="btn-group">
		...
		<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#tutupOrder">Tutup Order</button>
	</div>
<hr>
...
<div class="modal fade" id="tambahPetugas" role="dialog">
	<div class="modal-dialog modal-md">
		...
		<?php echo form_open('tiket/tambahPetugas');?>
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal">&times;</button>
				<h3 class="modal-title">Pilih Petugas</h3>
			</div>
			<div class="modal-body">
				<div id="petugas">
					<label>Petugas 1 :</label><br>
					<select class="form-control" name="petugas[]">
						<option>-- Pilih Petugas --</option>
					</select>
				</div><br>
				<input type="button" class="btn btn-default" value="Tambah Petugas" onClick="addPetugas('petugas')" />
			</div>
	...
	<div class="modal-dialog modal-md">
		...
		<?php echo form_open('tiket/baca');?>
			...
			<div class="modal-body">
				...
				<input type="text" name="tglClose" value="<?php echo date("Y-m-d");?>" readonly disabled><br>
				<label>Jam : </label><br>
				<input type="text" name="jamClose" value="<?php echo date("h:i:s");?>" readonly disabled>
			</div>
			<div class="modal-footer">
		...
		<?php
			if(!empty($volkerja)){
				...
				foreach ($volkerja as $row) {
					echo '<tr>';
					echo '<td>'.$no.'</td>';
					echo '<td>'.$row->tglBuat.'</td>';
					echo '<td>'.$row->pelapor.'</td>';
		...
		<div class="col-md-12">
			<div class="tabbable" id="tab1">
				<ul class="nav nav-tabs">
					...
					<li>
						<a href="#panel2" data-toggle="tab">Laporkan Volume Pekerjaan</a>
					</li>
					<li>
				...
				<div class="tab-content">
					...
					<div class="tab-pane" id="panel2">
						<div class="form">
							...
							<div class="col-md-6">
							<?php echo form_open('tiket/tambahVolKerja');?>
							<table class="table">
								<tbody id="dynamicInput">
									...
									<tr>
										<td><input class="form-control" type="text" name="namaItem[]"></td>
										<td><input class="form-control" type="text" name="jumlahItem[]"></td>
										<td><input class="form-control" type="text" name="satuanItem[]"></td>
									</tr>
								</tbody>
							</table>
							<input type="button" class="btn btn-default" value="Tambah Item" onClick="addInput('dynamicInput')" />
							<hr/>
							<button type="submit" class="btn btn-primary">
								Submit
							</button>
...
function addInput(divName){
	if(countera==limit){
		...
		newtr.innerHTML='<td><input class="form-control" type="text" name="namaItem[]"></td><td><input class="form-control" type="text" name="jumlahItem[]"></td><td><input class="form-control" type="text" name="satuanItem[]"></td>';
		document.getElementById(divName).appendChild(newtr);
		countera++;
	}
}

function addPetugas(petugas){
	if(counterb==limit){
		alert("Melebihi batas maksimum.");
	}else{
		var ptgs=document.createElement('div');
		ptgs.innerHTML='<label>Petugas '+ (counterb+1) +' :</label><br><select class="form-control" name="petugas[]"><option>-- Pilih Petugas --</option></select>';
		document.getElementById(petugas).appendChild(ptgs);
		counterb++;
	}
}
