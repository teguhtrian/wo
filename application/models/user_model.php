...
class User_model extends CI_Model{
	...
	public function getNippPegByDept($idDept){
		$this->db->select('nipp');
		$this->db->from('users');
		$this->db->where('users.idDepartemen',$idDept);
	...
	public function getPegByDept($idDept){
		$this->db->select('users.id, users.nipp, users.nama, users.idDepartemen, users.idRole, users.kodeUnit');
		$this->db->from('users');
		$this->db->where('users.idDepartemen',$idDept);
		$this->db->where('users.idRole >= 7');
		$this->db->order_by('users.idRole','ASC');
	...
	public function getSuperiorPusat(){
		$query = "SELECT * FROM users where idDepartemen='".$this->session->userdata('idDepartemen')."' and idRole='6'";
	...
	public function getKbid($iddept,$kodeUnit){
		$this->db->select('*');
		$this->db->from('users');
		$this->db->where('users.idDepartemen',$iddept);
		$this->db->where('users.kodeRole = "kbid"');
		$this->db->where('users.kodeUnit',$kodeUnit);
		$query = $this->db->get();
		return $query;
	}
	...
	public function get_by_dept($id_dept){
		$query = "SELECT us.id, us.nipp, us.nama, us.kodeRole, us.kodeUnit, us.idDepartemen, d.namaDepartemen, u.kodeUnit, u.namaUnit, us.lastLog
					FROM users AS us, unit AS u, departemen AS d
					WHERE us.idDepartemen = \"$id_dept\"
					AND us.kodeUnit = u.kodeUnit
					AND us.idDepartemen = d.id";
		$query = $this->db->query($query);
...
*/
	...
	// 	$this->db->where('users.kodeUnit',$kodeUnit);
	// 	$this->db->where('users.kodeRole <>',$ac);
	// 	$this->db->where('users.kodeRole <>',$ad);
	// 	$this->db->join('unit','unit.kodeUnit=users.kodeUnit','left');
	// 	$this->db->join('departemen','departemen.id=users.idDepartemen','left');
	// 	$this->db->join('role','role.id=users.idRole','left');
	// 	// $this->db->join('EmployeeWithCompleteAttributes','EmployeeWithCompleteAttributes.nipp=users.nipp COLLATE Latin1_General_CI_AS','left');
	// 	$query=$this->db->get();
	// 	//print_r($query);die;
	// 	return $query;
	// }
	...
	public function get_by_unit($kodeUnit){
		$query="EXEC getUserByUnit @kodeUnit = '".$kodeUnit."'";
	...
	public function user_by_dept($unit,$dept){
		//ambil data unit dari user
		//panggil data user sesuai unit yang ambil
		$query = $this->db->get_where('users',array('kodeUnit'=>$unit,'idDepartemen'=>$dept));
		return $query;
	}
	...
	public function getAbsenNippEachDate($nipp,$date){
		$query = $this->db->get_where('vLogInOutwDuration',array('nipp'=>$nipp,'Date'=>$date));
		return $query;
	}
}
?>