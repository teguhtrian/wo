...
class Tiket_model extends CI_model {
	...
	public function getDeptLiReportByUnit($kodeUnit){
		//$this->db->select('a.kodeUnit, a.namaUnit, sum(li.jenisInformasi=1 and li.idStatus!=4) as openInfo, sum(li.jenisInformasi=1 and li.idStatus=4) as closeInfo, sum(li.jenisInformasi=2 and li.idStatus!=4) as openPeng, sum(li.jenisInformasi=2 and li.idStatus=4) as closePeng');
		$this->db->select('a.kodeUnit, a.namaUnit, 
	...
	SUM(CASE WHEN li.jenisInformasi = 2 and li.idStatus = 4 THEN 1 ELSE 0 END) as closePeng');
		...
		$this->db->join('layananInformasi as li','li.kodeUnitTujuan=a.kodeUnit','left');
		//$this->db->where('a.kodeUnit',$kodeUnit);
		$this->db->group_by('a.namaUnit, a.kodeUnit');
		$this->db->order_by('a.kodeUnit','ASC');
...
*/
	...
	public function getPesanPengaduanById($id){
		$this->db->select('li.*, convert(varchar, li.waktuBuat, 120) as waktuBuat, uAsal.namaUnit as unitAsal, uTujuan.namaUnit as unitTujuan, dAsal.namaDepartemen as departemenAsal, dTujuan.namaDepartemen as departemenTujuan, u.nama as dari');
		$this->db->from('layananInformasi as li');
		$this->db->where('li.id',$id);
		$this->db->join('unit as uAsal','uAsal.kodeUnit=li.kodeUnitAsal','left');
		$this->db->join('unit as uTujuan','uTujuan.kodeUnit=li.kodeUnitTujuan','left');
