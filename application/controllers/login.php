...
class Login extends CI_controller
{
	...
	{
		$data['pesan'] = $this->session->flashdata('uji coba');
		$data['pesan'] = "";
		$data['unit'] = $this->unit_model->getUnitParent()->result();
		// $this->load->view('login_form',$data);
		$this->load->view('login_form2', $data);
	}
	...
	public function logout()
	{
		...
		//catat login logout
		$nama = str_replace("'", "''", $this->session->userdata('nama'));
		$this->actionLog($nama . " (" . $this->session->userdata('nipp') . ") Berhasil logout dari sistem", "logout()", current_url());
