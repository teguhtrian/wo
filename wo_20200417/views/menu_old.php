<li ><?php echo anchor('home','Beranda');?></li>
	...
	elseif($this->session->userdata('role')=='dir'||$this->session->userdata('role')=='kcab'||$this->session->userdata('role')=='kdiv'){
		echo '
			<li class="dropdown">
				...
				<ul class="dropdown-menu multi-level">
					...
					<li class="dropdown">
						<a tabindex="-1" href='.base_url().'report/reportOrderPegawaiDetail/'.'>Report Order Pegawai dengan Detail Tiket</a>
					</li>
					<!--
					<li class="dropdown">
						<a tabindex="-1" href="#">Order per Unit</a>
		...
		';
			...
			?>
	<?php 
		if($this->session->userdata('role')=='sa'||$this->session->userdata('role')=="ac"){ 
	...
	<li><?php echo anchor('user/logout','Logout', array('onclick' => "return confirm('Anda yakin akan logout?')"));?></li>